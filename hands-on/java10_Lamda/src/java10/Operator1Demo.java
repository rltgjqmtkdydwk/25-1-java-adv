...
public class Operator1Demo {
    public static void main(String[] args) {
        ...
        System.out.println(add2again.apply(3)); //apply로 전달

        IntUnaryOperator mul2 = x -> x*2;
        IntUnaryOperator add2mul2 = add2.andThen(mul2);
        System.out.printf("(3+2) * 2 = ");
        System.out.println(add2mul2.applyAsInt(3)); //andThen 사용

        IntBinaryOperator add = (x, y) -> x+y;
        System.out.println(add.applyAsInt(1, 2)); // 매개값 여러개도 전달

        List<Integer> list = new ArrayList<>();
        list.add(5);
        list.add(7);
        list.add(11);
        list.replaceAll(e -> e + 10); //replaceAll 사용
